...
case $CHOICE in
        1)
           aws ec2 describe-tags --filters "Name=tag:service,Values=*" --output text | awk '{print $5}' | sort | uniq #1>$PWD/TNT_service
            ;;
        2)
           aws ec2 describe-tags --filters "Name=tag:Service,Values=*" --output text | awk '{print $5}' | sort | uniq 1>$PWD/TNT_Service
            ;;
        3)
            aws ec2 describe-tags --filters "Name=tag:Type,Values=*" --output text | awk '{print $5}' | sort | uniq 1>$PWD/TNT_Type 
            ;;
        4)  
            aws ec2 describe-tags --filters "Name=tag:type,Values=*" --output text | awk '{print $5}' | sort | uniq 1>$PWD/TNT_type 
            ;;
